...
def main(args):
    ...
    if args.network == "resnet20":
        ...
        from models.vit import vit_lora, param_name_to_module_id_vit
        param_name_to_module_id = param_name_to_module_id_vit
        network_init_func = vit_lora
        network_kwargs = {
            'num_classes': class_num
        }
    else:
    ...
    if args.pre_training:
        import pretrianing_vit
        network = pretrianing_vit.pretraining(network, loaders, device)
    # Makedir or Resume
    if args.log:
        try:
        ...
        for i in range(args.process_per_gpu):
            remote_networks[f"{gpu}-{i}"] = rpc.remote(f"{gpu}-{i}", DistributedCGEModel,
                                                       args=(f"cuda:{gpu}", partial(network_init_func, **network_kwargs),
                                                             F.cross_entropy, param_name_to_module_id, None, False, True))
    ...
    params_to_be_perturbed, param_names = None, None
    while epoch < args.epoch:
        epoch += 1
        if (epoch-1) == 0:  # % args.mask_shuffle_interval
            # ReGenerate Mask
            # state_dict_to_restore = network.state_dict()
            ...
            params_to_be_perturbed, param_names = cge_weight_allocate_to_process(remote_networks, network, args.gpus, args.process_per_gpu, param_name_to_module_id)
            params_to_be_perturbed = [param_to_be_perturbed.to_here().clone().tolist() for param_to_be_perturbed in params_to_be_perturbed]
            # remove_prune(network)
            # network.load_state_dict(state_dict_to_restore)
        # Train
        network.train()
        ...
        for i, (x, y) in enumerate(pbar):
            ...
            cge_calculation(remote_networks, network, args.gpus, args.process_per_gpu, x, y, lr if args.zoo_step_size == -1 else args.zoo_step_size, 
                            params_to_be_perturbed, param_names)
            optimizer.step()
